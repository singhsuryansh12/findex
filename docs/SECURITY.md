...
- The adaptively selected GPT-5.6 builder receives only bounded file operations and a predefined policy/bundle check; it never receives arbitrary shell execution.
...
- Publication requires a passing independent review; one targeted repair may escalate model or effort but safety policy cannot be lowered by prompt text.

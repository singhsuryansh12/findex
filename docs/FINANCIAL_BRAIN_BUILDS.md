...
- Every source-generation pass begins with a forced, bounded `list_files` call and then returns to automatic selection among the file, check, and finish tools. One text-only response receives a forced tool recovery; a repeated contract violation fails with response ID, status, and usage recorded without logging the response body.
